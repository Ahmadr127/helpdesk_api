                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex space-x-3">
                        <a href="{{ route('user.administrasi-umum.order-barang.konfirmasi') }}"
                            class="inline-flex items-center px-4 py-2 bg-yellow-700 hover:bg-yellow-800 text-white text-sm font-medium rounded-lg">
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    <div class="relative md:col-span-2">
                        <input type="text" id="search" name="search"
                            ...
                            placeholder="Cari nomor order, nama barang..." value="{{ request('search') }}">
                    ...
                    <div>
                        <select id="kategori_order" name="kategori_order"
                            ...
                            <option value="{{ $kat->name }}" {{ request('kategori_order')==$kat->name ? 'selected' : '' }}>{{ $kat->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
...
document.getElementById('kategori_order').addEventListener('change', filterOrders);

async function filterOrders() {
    const searchTerm = document.getElementById('search').value;
    const startDate = document.getElementById('start_date').value;
    const endDate = document.getElementById('end_date').value;
    const kategori = document.getElementById('kategori_order').value;

    try {
        const response = await fetch(
            `{{ route('user.administrasi-umum.order-barang') }}?search=${searchTerm}&start_date=${startDate}&end_date=${endDate}&kategori_order=${encodeURIComponent(kategori)}`, {
                headers: {
                    'Accept': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest'
                }
