...
@section('title', 'Permintaan Barang - Konfirmasi')
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="mb-6 bg-gradient-to-r from-green-600 to-blue-300 rounded-lg p-6 shadow-sm">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="mt-2 text-sm text-white">Daftar permintaan barang yang telah dikonfirmasi</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm">
            ...
            <div id="emptyState" class="hidden p-12 text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 mb-2">Tidak Ada Permintaan Dikonfirmasi</h3>
                <p class="text-gray-500">Belum ada permintaan yang dikonfirmasi saat ini</p>
            </div>
...
async function refreshOrderList() {
    try {
        const response = await fetch('{{ route("user.administrasi-umum.order-barang.konfirmasi") }}', {
            headers: {
...
document.getElementById('filterDate').addEventListener('click', async function() {
    ...
    try {
        const response = await fetch(
            `{{ route('user.administrasi-umum.order-barang.konfirmasi') }}?start_date=${startDate}&end_date=${endDate}`, {
                headers: {
                    'Accept': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest'
                }
